...
public class TeamLevelImpl {
	private static final Logger log = LoggerFactory.getLogger(TeamLevelImpl.class);
	public static void handleAward(List<PlayerTaskInfo> all, int dif_day) {
		...
		if (dif_index < 0)
			return;
		log.info("TeamLevelImpl.handleAward() dif day {}", dif_day);
		long now = Calendar.getInstance().getTimeInMillis();
		Map<Integer, String> rank = RankManager.team_lv_rank;
		List<String> tops = new ArrayList<String>();
		Hdrankinglv other_player_cfg = null;
		for (Entry<Integer, Hdrankinglv> a_cfg : ConfigConstant.tHdrankinglv.entrySet()) {
			if (a_cfg.getKey() == 0) {
				other_player_cfg = a_cfg.getValue();
				continue;
			}
			Hdrankinglv config = a_cfg.getValue();
			...
			for (int i = begin; i <= end; ++i)
			{
				...
				if (p_info == null)
					continue;
				if (!ActivityType.checkActivityOpen(ActivityType.LvRank, now, p_info))
					continue;
