...
@Service
@Scope("prototype")
public class EnchantInvestImpl {
	private static final Logger log = LoggerFactory.getLogger(EnchantInvestImpl.class);
	
    public final static EnchantInvestImpl getInstance(){
        return SpringContextUtil.getBean(EnchantInvestImpl.class);
	}
    
	@Autowired
	PlayerEnchantInvestDB m_act_db;
	PlayerEnchantInvestInfo m_act;
    ...
    public CommonMsg handleInfo(CommonMsg respond) {
    	...
    	if (!m_act.has_invest) {
    		if (!ActivityType.checkActivityOpen(ActivityType.EnchantInvest, Calendar.getInstance().getTimeInMillis(), m_player)) {
        		return respond;
        	}
    	}
    ...
    public CommonMsg handleBuy(CommonMsg respond) {
    	...
    	if (m_player.vip_level < ConfigConstant.tConf.getInvestEnchant()[0]) {
    		respond.header.rt_sub = 1129;
    		return respond;
    	}
    	
    	int cost_dmd = ConfigConstant.tConf.getInvestEnchant()[1];
    ...
    public CommonMsg handleReward(CommonMsg respond, CommonMsg receive)
    {
		...
		int[] role_ids = ConfigConstant.tConf.getInvestEnchantID();
		boolean found = false;
