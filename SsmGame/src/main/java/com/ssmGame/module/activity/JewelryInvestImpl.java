...
@Service
@Scope("prototype")
public class JewelryInvestImpl {
	private static final Logger log = LoggerFactory.getLogger(JewelryInvestImpl.class);
	
    public final static JewelryInvestImpl getInstance(){
        return SpringContextUtil.getBean(JewelryInvestImpl.class);
	}
    
	@Autowired
	PlayerJewelryInvestDB m_act_db;
	PlayerJewelryInvestInfo m_act;
    ...
    public CommonMsg handleInfo(CommonMsg respond) {
    	...
    	if (!m_act.has_invest) {
    		if (!m_act.has_invest) {
        		if (!ActivityType.checkActivityOpen(ActivityType.JewelryInvest, Calendar.getInstance().getTimeInMillis(), m_player)) {
            		return respond;
            	}
        	}
    	}
    ...
    public CommonMsg handleBuy(CommonMsg respond) {
    	...
    	if (m_player.vip_level < ConfigConstant.tConf.getInvestJewelry()[0]) {
    		respond.header.rt_sub = 1129;
    		return respond;
    	}
    	
    	int cost_dmd = ConfigConstant.tConf.getInvestJewelry()[1];
    ...
    public CommonMsg handleReward(CommonMsg respond, CommonMsg receive)
    {
		...
		int[] role_ids = ConfigConstant.tConf.getInvestJewelryID();
		boolean found = false;
