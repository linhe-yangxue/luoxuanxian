...
@Service("SdkmwyxImpl")
@Scope("prototype")
public class SdkMwyxImpl implements I_Platform {
	private static final Logger log = LoggerFactory.getLogger(SdkMwyxImpl.class);
	private static final String PAY_URL = "http://www.acg5.com/media.php/Game/game_pay/?";
	...
	public UserBase logVerification(Map<String, Object> param, DataConf dSource) throws Exception {
		// log.info(JsonTransfer.getJson(param));
		...
		// 没有发这个 temp_param.put("channelExt",
		// param.get("channelExt").toString());
		...
		// sb.append(temp_param.get("channelExt")).append("|")
		sb.append(temp_param.get("user_id")); // 透传，需要将来用
		param.put("extend", sb.toString());
		// log.info("loging success {}", temp_param.get("user_id"));
		return ubase;
	}
	...
	public Object payVerification(DataConf dSource) throws Exception {
		...
		String user_id = (String) dSource.params.get("extend");
		// String user_id = extend.substring(extend.indexOf("|") + 1);
		// String channelExt = extend.substring(0, extend.indexOf("|") + 1);
		// log.info("{} {} {}", extend, user_id, channelExt);
		Map<String, String> temp_param = new LinkedHashMap<String, String>();
		temp_param.put("game_appid", dSource.doc.getPayKey());
