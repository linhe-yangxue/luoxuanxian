...
public class FormatHTMLLayout extends HTMLLayout {   
   ...
   if (locationInfo) {   
        ...
        if (s != null) {   
            ...
            appendThrowableAsHTMLs(s, sbuf);   
            sbuf.append("</td></tr>" + Layout.LINE_SEP);   
        }   
        return sbuf.toString();   
    }   
  
    private void appendThrowableAsHTMLs(String[]s, StringBuffer sbuf) {   
        if (s != null) {   
            int len = s.length;   
            if (len == 0)   
                return;   
    ...
    public String getHeader() {   
        ...
        if (locationInfo) {   
            sbuf.append("<th>File</th>" + Layout.LINE_SEP);   
        }   
   
        sbuf.append("<th>info</th>" + Layout.LINE_SEP);   
        sbuf.append("</tr>" + Layout.LINE_SEP);   
